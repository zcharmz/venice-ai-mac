...
Venice AI is one of the only AI platforms that genuinely does not log your conversations or use them for training. There is no official desktop app, so every session starts in a browser tab. This app fixes that. It is a proper `.app` that lives in your Applications folder, remembers your login, and opens instantly to the chat interface.
...
- **No telemetry, no analytics, no phoning home.** The wrapper code has nothing that contacts anywhere except `venice.ai`
...
- **Venice AI itself does not log your chats.** That is the whole point of using Venice. Conversations are processed on-device or on privacy-preserving infrastructure and are not stored or used for training
- **Open source.** Every line of code in this repo is readable. There is nothing hidden

The only data stored locally is your window size and position (`window-state.json` in the app's support folder) so the window reopens where you left it.
...
3. Right-click the app and select **Open** the first time (bypasses Gatekeeper for unsigned apps)
4. Log in and you will stay logged in from then on

**If macOS says the app is damaged or cannot be verified,** run this in Terminal and then open it again:

```bash
xattr -cr "/Applications/Venice AI.app"
```
...
```bash
# Clone
git clone https://github.com/zcharmz/venice-ai-mac.git
cd venice-ai-mac
...
- External URLs (anything not on `venice.ai`) are intercepted and forwarded to your system browser instead of opening a new Electron window
